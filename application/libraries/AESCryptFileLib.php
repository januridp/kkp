...
class AESCryptFileLib
{
	...
	{
		//Cek kita bisa membaca source file
		$this->checkSourceExistsAndReadable($source_file);

		//Periksa apakah ext_data diformat dengan benar
		$this->checkExtensionData($ext_data);
		
		//Periksa apakah password adalah string (tidak bisa NULL)
		$this->checkPassphraseIsValid($passphrase);
		
		//Lalu lakukan enkripsi disini
		$dest_fh = $this->doEncryptFile($source_file, $passphrase, $dest_file, $ext_data);
		
		//Kembalikan lokasi file terenkripsi
	...
	public function readExtensionBlocks($source_file)
	{
		...
		//Mencoba untuk mengurai dan mengembalikan blok ekstensi saja
		//Buka file
		...
		if (bin2hex($version_chunk) === dechex(ord("0"))) {
			//File ini menggunakan versi 0 dari standar
			//Blok ekstensi tidak ada di spec versi ini
			$extension_blocks = NULL;
		} else if (bin2hex($version_chunk) === dechex(ord("1"))) {
			//File ini menggunakan versi 1 dari standar
			//Blok ekstensi tidak ada di spec versi ini
			$extension_blocks = NULL;
		} else if (bin2hex($version_chunk) === dechex(ord("2"))) {
			
			//File ini menggunakan versi 2 standar (standar terbaru pada saat penulisan)
